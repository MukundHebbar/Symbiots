...
// ---------- DHT11 Configuration ----------
#define DHTPIN1 17  // Flammable compartment
#define DHTPIN2 22  // Cold storage compartment
...
void publishDHTData() {
  ...
  // Check thresholds and toggle LEDs
  if (temp1 > 45.0) {
    digitalWrite(FLAME_LED, HIGH);
    Serial.println("⚠️ Flammable Temp HIGH: LED ON");
  } else {
    digitalWrite(FLAME_LED, LOW);
  }
  ...
  if (temp2 < 8.0) {
    ...
    Serial.println("❄️ Cold Temp LOW: LED ON");
  } else {
    digitalWrite(COLD_LED, LOW);
  }
