...
const handleAddUser = async () => {
  ...
  return (
    ...
    <div className="flex gap-3 mb-6">
        <input
            className="border border-gray-600 bg-gray-800 text-white p-3 rounded-lg shadow-md focus:outline-none focus:ring-2 focus:ring-blue-400 transition-all duration-200"
            type="text"
            value={name}
        ...
        />
        <button 
            className="bg-gradient-to-r from-blue-500 to-blue-400 hover:from-blue-400 hover:to-blue-300 px-5 py-3 rounded-lg font-semibold shadow-lg transition-all duration-300"
        ...
        <ul className="w-full max-w-lg bg-gray-800 p-5 rounded-xl shadow-lg border border-gray-700">
            {users.length === 0 ? (
                ...
                users.map((user) => (
                    <li 
                        key={user.id} 
                        className="flex justify-between items-center p-3 bg-gray-700 rounded-lg mb-3 shadow-md hover:bg-gray-600 transition-all duration-200 transform hover:scale-[1.02]"
                    >
